          ...
          <div className="bg-white border border-[#E8E8E8] rounded-[14px] overflow-hidden">
              ...
              {(order.items || []).map((item, idx) => (
                <div key={idx} className="flex gap-4 border-b border-[#F0F0F0] last:border-0 pb-4 last:pb-0 mb-4 last:mb-0">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-body text-[16px] text-[#0A0A0A] font-medium">{item.name || item.product?.name || 'Product'}</h3>
                      <p className="font-body text-[13px] text-[#999999] mt-1">Size: <span className="text-[#0A0A0A]">{item.selectedSize || item.size || 'M'}</span></p>
                    </div>
                    <div className="flex items-end justify-between">
                      <p className="font-body text-[14px] text-[#555555]">Qty: {item.quantity}</p>
                      <p className="font-mono text-[16px] text-[#0A0A0A] font-bold">₹{((item.price || item.product?.price || 0) * item.quantity).toLocaleString('en-IN')}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-[#F8F8F6] p-6 border-t border-[#E8E8E8]">
              <div className="flex justify-between items-center mb-2">
                ...
                <p className="font-mono text-[14px] text-[#0A0A0A]">₹{(order.itemsPrice || order.totalPrice || 0).toLocaleString('en-IN')}</p>
              </div>
              <div className="flex justify-between items-center mb-4">
                <p className="font-body text-[14px] text-[#555555]">Shipping</p>
                <p className="font-body text-[14px] text-[#10B981]">{order.shippingPrice ? `₹${order.shippingPrice}` : 'Free'}</p>
              </div>
              <div className="flex justify-between items-center pt-4 border-t border-[#E8E8E8]">
                ...
                <p className="font-mono text-[24px] text-[#0A0A0A] font-bold">₹{(order.totalPrice || order.total || 0).toLocaleString('en-IN')}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white border border-[#E8E8E8] rounded-[14px] p-6">
            ...
            {order.shippingAddress ? (
              <div className="font-body text-[14px] text-[#555555] space-y-1">
                <p className="text-[#0A0A0A] font-medium">{order.shippingAddress.fullName || order.shippingAddress.name}</p>
