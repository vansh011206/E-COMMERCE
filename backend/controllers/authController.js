...
export const registerUser = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      res.status(201).json({
        _id: user._id, name: user.name, email: user.email, isAdmin: user.isAdmin, addresses: user.addresses || [], wishlist: [], token
      });
    } else {
...
export const authUser = async (req, res) => {
  try {
    ...
    if (user && (await user.matchPassword(password))) {
      const token = generateToken(res, user._id);
      res.json({
        _id: user._id, name: user.name, email: user.email, isAdmin: user.isAdmin, addresses: user.addresses || [], wishlist: user.wishlist || [], token
      });
    } else {
