...
const OrderCard = ({ order }) => {
  ...
  const handleReorder = () => {
    order.items.forEach(item => {
      // Mock product structure for reorder
      addToCart({ id: item.name, name: item.name, price: order.total / order.items.length, images: [item.image] }, item.size || 'M', null, item.quantity);
    });
  ...
  const currentStepIdx = steps.findIndex(s => s.toUpperCase() === order.status.toUpperCase());
  ...
  return (
    <div className="bg-white border border-[#E8E8E8] rounded-xl p-5 mb-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-mono text-[13px] text-[#0A0A0A] font-semibold">Order {order.id}</h3>
          <p className="font-body text-[12px] text-[#999] mt-0.5">Placed on {order.date}</p>
        </div>
        <div className={`px-3 py-1 rounded-full font-heading text-[11px] font-bold uppercase tracking-wider ${getStatusColor(order.status)}`}>
          {order.status}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          <p className="font-mono text-[14px] text-[#0A0A0A] font-bold">₹{order.total.toLocaleString('en-IN')}</p>
        </div>
      </div>
      ...
      {order.status.toUpperCase() !== 'CANCELLED' && (
        <div className="relative flex justify-between mb-6 px-2">
          <div className="absolute left-4 right-4 top-1.5 h-[1.5px] bg-[#E8E8E8] -z-10" />
          <div className="absolute left-4 top-1.5 h-[1.5px] bg-[#0A0A0A] -z-10 transition-all duration-500" style={{ width: `calc(${(activeIdx / (steps.length - 1)) * 100}% - 16px)` }} />
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <div key={step} className="flex flex-col items-center gap-2 bg-white px-1">
                <div className={`relative w-3.5 h-3.5 rounded-full flex items-center justify-center ${isCompleted ? 'bg-[#0A0A0A]' : isCurrent ? 'bg-[#FF3C78]' : 'bg-[#E8E8E8]'}`}>
                  {isCurrent && <div className="absolute -inset-1 border border-[#FF3C78] rounded-full animate-ping opacity-20" />}
                </div>
                <span className={`font-body text-[11px] ${isCurrent ? 'text-[#0A0A0A] font-medium' : 'text-[#999]'}`}>{step}</span>
              </div>
            );
          })}
        </div>
              ...
              {order.items.map((item, idx) => (
                <div key={idx} className="flex gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-body text-[12px] text-[#999] mt-0.5">Size: {item.size || 'M'} | Qty: {item.quantity}</p>
                  </div>
                </div>
              ))}
...
const Orders = () => {
  const { isAuthenticated, orders } = useAuthStore();
